...
Multi page structured website with DB Integration
...
Open `css/style.css` and look for the `:root` block near the top of the file:
```css
:root {
...
Open `css/style.css` and find `font-family` on the `body` selector.
...
For background images defined in CSS, open `css/style.css` and search for `background-image`.
